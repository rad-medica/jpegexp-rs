...
class TestCodecIntegration(unittest.TestCase):
    ...
    def test_jpeg2000_encode_grayscale(self):
        output_j2k = os.path.join(self.temp_dir, "encoded_gray.jp2")
        args = [
            "encode",
            "-i", self.gray_raw_path,
            "-o", output_j2k,
        ...
        try:
            # imagecodecs decode
            # Note: imagecodecs.imread usually handles jp2 if openjpeg linked
            ...
            # RMSE/PSNR (lossless or near?) 
            # Default implementation might be lossless or lossy depending on args.
            # Assuming lossless for now if not specified otherwise.
            # But J2K usually lossy default?
            # Let's check MSE.
    ...
    def test_jpeg2000_encode_rgb(self):
        output_j2k = os.path.join(self.temp_dir, "encoded_rgb.jp2")
        args = [
            "encode",
            "-i", self.rgb_raw_path,
            "-o", output_j2k,
