...
TEST_DIR = Path("tests/fixtures/jpegls")
...
def generate_test_image(name, width, height, channels, bit_depth=8, pattern="gradient"):
    ...
    print(
        f"Generating {name}: {width}x{height}x{channels} @ {bit_depth}bit, pattern={pattern}"
    )
    ...
    if pattern == "gradient":
        if channels == 1:
            # Grayscale gradient
            data = np.linspace(0, max_val, width * height, dtype=dtype).reshape(
                (height, width)
            )
        elif channels == 3:
            # RGB gradients
            ...
            data[:, :, 0] = np.linspace(
                0, max_val, width * height, dtype=dtype
            ).reshape((height, width))  # R
            data[:, :, 1] = (
                np.linspace(0, max_val, height * width, dtype=dtype)
...
def save_test_case(data, name):
    ...
    try:
        ...
        if np.array_equal(data, decoded):
            print(f"  [OK] CharLS roundtrip: PASS")
        else:
            print(
                f"  [X] CharLS roundtrip: FAIL (max diff: {np.max(np.abs(data.astype(int) - decoded.astype(int)))})"
            )
        ...
        print(f"  [X] CharLS encode failed: {e}")
        return False
...
def main():
    ...
    for test_case in test_cases:
        ...
        try:
            data = generate_test_image(
                name, width, height, channels, bit_depth, pattern
            )
            ...
            print(f"  [X] Failed to generate {name}: {e}")
            fail_count += 1
        print()
    ...
    with open(summary_path, "w") as f:
        ...
        for test_case in test_cases:
            name, width, height, channels, bit_depth, pattern = test_case
            f.write(
                f"| {name} | {width}x{height} | {channels} | {bit_depth} | {pattern} |\n"
            )
        f.write("\n## Files\n\n")
