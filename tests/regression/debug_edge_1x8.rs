...
fn debug_edge_1x8() {
    ...
    let jls_data = fs::read("tests/fixtures/jpegls/edge_1x8_gray.jls")
        .expect("Failed to read edge_1x8_gray.jls");
    
    // Load expected data
    let expected = fs::read("tests/fixtures/jpegls/edge_1x8_gray.raw")
        .expect("Failed to read edge_1x8_gray.raw");
    ...
    println!("Frame info: {}x{}, {} components, {} bits", 
        info.width, info.height, info.component_count, info.bits_per_sample);
    ...
    match decoder.decode(&mut decoded) {
        Ok(_) => {
            ...
            for (i, (exp, got)) in expected.iter().zip(decoded.iter()).enumerate() {
                if exp != got {
                    println!("  Byte {}: expected {}, got {} (diff={})", 
                        i, exp, got, (*got as i32 - *exp as i32));
                }
            }
        }
        Err(e) => {
