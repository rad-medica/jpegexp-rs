...
#[test]
#[ignore]
fn debug_16bit_simple() {
    let width = 8;
    let height = 8;
    
    let mut pixels_u16 = vec![32768u16; width * height];
    
    let mut pixels = Vec::with_capacity(width * height * 2);
    for val in pixels_u16.iter() {
        pixels.push((val & 0xFF) as u8);
    ...
    println!("\n=== Testing 8x8 16-bit solid (32768) with 1 decomposition level ===\n");
    ...
    fs::write("debug_ours_8x8_16bit.j2k", our_bytes).unwrap();
    
    fs::write("debug_input_8x8_16bit.raw", &pixels).unwrap();
    let status = Command::new("libs/bin/opj_compress.exe")
        .args(&[
            "-i", "debug_input_8x8_16bit.raw",
            "-o", "debug_opj_8x8_16bit.j2k",
            "-n", "2",
            "-r", "1",
            "-F", "8,8,1,16,u",
            "-I",
    ...
    let opj_bytes = fs::read("debug_opj_8x8_16bit.j2k").unwrap();
    
    println!("--- File Sizes ---");
    println!("Ours:     {} bytes", our_size);
    println!("OpenJPEG: {} bytes", opj_bytes.len());
    
    let our_decoded_pnm = "debug_ours_decoded_8x8_16bit.pgm";
    let opj_decoded_pnm = "debug_opj_decoded_8x8_16bit.pgm";
    
    let status1 = Command::new("libs/bin/opj_decompress.exe")
        .args(&["-i", "debug_ours_8x8_16bit.j2k", "-o", our_decoded_pnm])
        .output()
        .expect("Failed to decode our file");
    
    let status2 = Command::new("libs/bin/opj_decompress.exe")
        .args(&["-i", "debug_opj_8x8_16bit.j2k", "-o", opj_decoded_pnm])
        .output()
    ...
    fn parse_pnm_16bit(data: &[u8]) -> Vec<u16> {
        let mut offset = 0;
        ...
        pixel_data.chunks_exact(2).map(|chunk| {
            u16::from_be_bytes([chunk[0], chunk[1]])
        }).collect()
    }
    
    let our_pixels_decoded = parse_pnm_16bit(&our_decoded_data);
    let opj_pixels_decoded = parse_pnm_16bit(&opj_decoded_data);
    ...
    for i in 0..pixel_count {
        let original = pixels_u16[i] as i32;
    ...
    assert_eq!(mae_ours, 0.0, "16-bit solid pattern should be perfectly lossless!");
}
