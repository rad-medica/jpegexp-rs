...
fn test_htj2k_decoder_openhtj2k_interop() {
    ...
    let enc_bin = find_binary("open_htj2k_enc").expect("open_htj2k_enc not found");
    let output = Command::new(enc_bin)
        .args(
            &[
                "-i",
                "test_htj2k_input.pgm",
                "-o",
                "test_htj2k.j2c",
                "Creversible=yes",
            ],
    ...
    assert_eq!(
        mae,
        0.0,
        "HTJ2K decoder should perfectly decode OpenHTJ2K-encoded files"
    );
}
...
fn test_htj2k_encoder_openhtj2k_decoder() {
    ...
    // Save encoded stream
    fs::write("tests/fixtures/out/test_our_htj2k.jhc", &output)
        .expect("Failed to write encoded file");

    // Decode with OpenHTJ2K
    let dec_bin = find_binary("open_htj2k_dec").expect("open_htj2k_dec not found");
    let decode_output = Command::new(dec_bin)
        .args(
            &[
                "-i",
                "tests/fixtures/out/test_our_htj2k.jhc",
                "-o",
                "test_our_htj2k.pgm",
            ],
        )
    ...
    // Clean up
    let _ = fs::remove_file("tests/fixtures/out/test_our_htj2k.jhc");
    let _ = fs::remove_file("test_our_htj2k.pgm");

    assert_eq!(
        mae,
        0.0,
        "HTJ2K encoder should be compatible with OpenHTJ2K decoder"
    );
}
...
fn test_htj2k_vs_j2k_performance() {
    ...
    if output.status.success() {
        ...
        println!(
            ...
            (width * height) as f64 / htj2k_size as f64
        );
    }

    // Test OpenJPEG encoder (classic JPEG 2000)
    let start = std::time::Instant::now();
    let opj_bin = find_binary("opj_compress").expect("opj_compress not found");
    let output = Command::new(opj_bin)
        .args(
            &[
                "-i",
                "test_perf_input.pgm",
    ...
    if output.status.success() {
        ...
        println!(
            "  Compression ratio: {:.2}:1",
            (width * height) as f64 / j2k_size as f64
        );
    }
...
fn test_htj2k_marker_constants() {
    ...
    assert_eq!(
        JpegMarkerCode::Capability as u8,
        0x50,
        "CAP marker should be 0xFF50 (0x50 after 0xFF prefix)"
    );
    ...
    assert_eq!(
        JpegMarkerCode::QuantizationDefault as u8,
        0x5C,
        "QCD marker"
    );
}
