...
fn test_jpeg1_quality_determinism() {
    ...
    for &quality in &qualities {
        ...
        let encoded_size2 = encoder2.encode(&pixels, &frame_info, &mut encoded2)
...
fn test_jpeg1_compression_consistency() {
    ...
    let size1 = encoder.encode(&pixels1, &frame_info, &mut encoded1)
    ...
    let mut encoded2 = vec![0u8; width * height * 2];
    let size2 = encoder2.encode(&pixels2, &frame_info, &mut encoded2)
...
fn test_jpeg1_interop_edge_cases() {
    ...
    for (width, height, name) in test_cases {
        ...
        let encoded_size = encoder.encode(&pixels, &frame_info, &mut encoded)
            .expect(&format!("Encoding {} failed", name));
        
        let mut decoder = Jpeg1Decoder::new(&encoded[..encoded_size]);
        decoder.read_header().expect(&format!("Reading header {} failed", name));
        
        let mut decoded = vec![0u8; (width * height) as usize];
        decoder.decode(&mut decoded).expect(&format!("Decoding {} failed", name));
