...
fn test_optimized_huffman_reduces_file_size() {
    ...
    let len_standard = encoder_standard
        .encode(&rgb, &frame_info, &mut jpeg_standard)
    ...
    let len_optimized = encoder_optimized
        .encode(&rgb, &frame_info, &mut jpeg_optimized)
    ...
    let reduction_percent = 100.0 *
        (1.0 - jpeg_optimized.len() as f64 / jpeg_standard.len() as f64);
    ...
    assert!(
        ...
        "Expected at least 3% reduction, got {:.2}%",
        reduction_percent
    );
}
...
fn test_optimized_huffman_maintains_quality() {
    ...
    let len_encoded = encoder.encode(&rgb, &frame_info, &mut encoded).expect(
        "Encode failed",
    );
    ...
    let len_std = encoder_std
        .encode(&rgb, &frame_info, &mut encoded_std)
        .expect("Standard encode failed");
    ...
    decoder_std.decode(&mut decoded_std).expect(
        "Decode std failed",
    );
