...
fn test_encode_decode(name: &str, pixels: &[u8], width: u32, height: u32, dwt_levels: u8) -> f64 {
    ...
    let len = encoder.encode(pixels, &info, &mut output).expect(&format!(
        "Failed to encode {}",
        name
    ));
    output.truncate(len);
...
fn generate_checkerboard(width: u32, height: u32, square_size: u32) -> Vec<u8> {
    ...
    for y in 0..height {
        for x in 0..width {
            let idx = (y * width + x) as usize;
            pixels[idx] = if ((x / square_size) + (y / square_size)) % 2 == 0 {
                0
            } else {
                255
            };
