...
fn test_j2k_lossy_8bit_quality_levels() {
    ...
    for (&quality, &threshold) in quality_levels.iter().zip(mae_thresholds.iter()) {
        ...
        let image = decoder.decode().expect(
            &format!("Decoding Q{} failed", quality),
        );
        let decoded = image.reconstruct_pixels().expect(
            "Pixel reconstruction failed",
        );
        ...
        assert!(
            mae < threshold,
            "Quality {} MAE {:.4} exceeds threshold {:.1}",
            quality,
            mae,
            threshold
        );
...
fn test_j2k_color_rgb_lossy() {
    ...
    let decoded = image.reconstruct_pixels().expect(
        "RGB lossy reconstruction failed",
    );
...
fn test_j2k_decomposition_levels() {
    ...
    for &level in &dwt_levels {
        ...
        let mut decoder = J2kDecoder::new(&mut reader);
        let image = decoder.decode().expect(
            &format!("Decoding DWT {} failed", level),
        );
        let decoded = image.reconstruct_pixels().expect(
            "Pixel reconstruction failed",
        );
...
fn test_j2k_multi_tile_support() {
    ...
    let decoded = image.reconstruct_pixels().expect(
        "Multi-tile reconstruction failed",
    );
