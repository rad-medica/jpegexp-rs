...
fn test_420_subsampling_encode_decode() {
    ...
    let enc_len_420 = encoder_420
        .encode(&source, &frame_info, &mut encoded_420)
    ...
    let enc_len_444 = encoder_444
        .encode(&source, &frame_info, &mut encoded_444)
        .expect("4:4:4 encode failed");

    // Verify 4:2:0 produces smaller file
    println!(
        "4:2:0 size: {} bytes, 4:4:4 size: {} bytes",
        enc_len_420,
        enc_len_444
    );
    ...
    assert!(
        size_ratio < 0.85,
        "4:2:0 file not significantly smaller: {:.2}%",
        size_ratio * 100.0
    );
    assert!(
        size_ratio > 0.45,
        "4:2:0 file unexpectedly small: {:.2}%",
        size_ratio * 100.0
    );
...
fn test_422_subsampling_encode_decode() {
    ...
    let enc_len_422 = encoder_422
        .encode(&source, &frame_info, &mut encoded_422)
    ...
    println!(
        "4:2:2 size: {} bytes, 4:4:4 size: {} bytes",
        enc_len_422,
        enc_len_444
    );
    ...
    assert!(
        size_ratio < 0.98,
        "4:2:2 file not smaller: {:.2}%",
        size_ratio * 100.0
    );
    assert!(
        size_ratio > 0.60,
        "4:2:2 file unexpectedly small: {:.2}%",
        size_ratio * 100.0
    );
...
fn test_444_no_subsampling() {
    ...
    let enc_len = encoder.encode(&source, &frame_info, &mut encoded).expect(
        "4:4:4 encode failed",
    );
