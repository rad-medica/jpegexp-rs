...
fn roundtrip_test(width: u32, height: u32, depth: u8, components: u8) {
    ...
    // Generate data
    let mut pixels =
        vec![0u8; (width * height * components as u32 * if depth > 8 { 2 } else { 1 }) as usize];
    ...
    let output_path = std::path::Path::new(
        "C:/Users/aroja/CODE/jpegexp-rs/test_htj2k_2x2_gradient_ours.j2k",
    );
