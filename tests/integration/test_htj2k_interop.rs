...
fn test_htj2k_encoder_basic_roundtrip() {
    ...
    let decoded = image.reconstruct_pixels().expect(
        "HTJ2K reconstruction failed",
    );
...
fn test_htj2k_cap_marker_presence() {
    ...
    let len_htj2k = encoder_htj2k
        .encode(&pixels, &frame_info, &mut output_htj2k)
    ...
    let len_j2k = encoder_j2k
        .encode(&pixels, &frame_info, &mut output_j2k)
        .expect("J2K encode failed");

    // Verify CAP marker (0xFF50) presence
    let has_cap_htj2k = output_htj2k[..len_htj2k].windows(2).any(|w| {
        w[0] == 0xFF && w[1] == 0x50
    });
    let has_cap_j2k = output_j2k[..len_j2k].windows(2).any(|w| {
        w[0] == 0xFF && w[1] == 0x50
    });
    ...
    let has_soc_htj2k = output_htj2k[..len_htj2k].windows(2).any(|w| {
        w[0] == 0xFF && w[1] == 0x4F
    });
    let has_soc_j2k = output_j2k[..len_j2k].windows(2).any(|w| {
        w[0] == 0xFF && w[1] == 0x4F
    });
...
fn test_htj2k_lossless_8bit() {
    ...
    // Verify CAP marker
    let has_cap = encoded[..encoded_size].windows(2).any(|w| {
        w[0] == 0xFF && w[1] == 0x50
    });
