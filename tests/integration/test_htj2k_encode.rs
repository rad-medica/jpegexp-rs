...
fn test_htj2k_encode_basic() {
    ...
    let len_std = encoder_std
        .encode(&original, &frame_info, &mut encoded_std)
        .expect("Std encode failed");
    ...
    let len_ht = encoder_ht
        .encode(&original, &frame_info, &mut encoded_ht)
        .expect("HT encode failed");
    ...
    match decoder.decode() {
        Ok(image) => {
            ...
            match image.reconstruct_pixels() {
                ...
                Err(e) => {
                    println!(
                        "Reconstruction failed (expected if decoder incomplete): {}",
                        e
                    )
                }
            }
