...
fn test_monochrome1_8bit_lossless() {
    ...
    assert!(
        mono1_pixels[0] >= 254,
        "First pixel should be near white (255), got {}",
        mono1_pixels[0]
    );
    assert!(
        mono1_pixels[mono1_pixels.len() - 1] <= 1,
        "Last pixel should be near black (0), got {}",
        mono1_pixels[mono1_pixels.len() - 1]
    );
...
fn test_monochrome1_12bit_lossless() {
    ...
    assert!(
        mono1_pixels[0] >= 4090,
        "First pixel should be near white (4095), got {}",
        mono1_pixels[0]
    );
    assert!(
        mono1_pixels[mono1_pixels.len() - 1] <= 20,
        "Last pixel should be near black (0), got {}",
        mono1_pixels[mono1_pixels.len() - 1]
    );
...
fn test_monochrome1_16bit_lossless() {
    ...
    // Verify inversion
    assert!(
        mono1_pixels[0] >= 65000,
        "First pixel should be near white (65535), got {}",
        mono1_pixels[0]
    );
    assert!(
        mono1_pixels[mono1_pixels.len() - 1] <= 300,
        "Last pixel should be near black (0), got {}",
        mono1_pixels[mono1_pixels.len() - 1]
    );
...
fn test_monochrome1_inversion_symmetry() {
    ...
    assert_eq!(
        original[width * height - 1],
        restored[width * height - 1],
        "Last pixel should match"
    );
