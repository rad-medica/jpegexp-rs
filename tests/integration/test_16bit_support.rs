...
fn test_16bit_lossless_nuclear_pattern() {
    ...
    let decoded_u8 = image.reconstruct_pixels().expect(
        "16-bit nuclear reconstruction failed",
    );
    let decoded_u16 = u8_to_u16_le(&decoded_u8);
    ...
    assert_eq!(
        mae,
        0.0,
        "16-bit lossless nuclear pattern should have MAE=0"
    );
...
fn test_16bit_lossless_checkerboard() {
    ...
    let image = decoder.decode().expect(
        "16-bit checkerboard decoding failed",
    );
    let decoded_u8 = image.reconstruct_pixels().expect(
        "16-bit checkerboard reconstruction failed",
    );
    let decoded_u16 = u8_to_u16_le(&decoded_u8);
...
fn test_16bit_lossy_q85() {
    ...
    let decoded_u8 = image.reconstruct_pixels().expect(
        "16-bit Q85 reconstruction failed",
    );
    let decoded_u16 = u8_to_u16_le(&decoded_u8);
    ...
    assert!(
        mae < 500.0,
        "16-bit Q85 should have MAE < 500, got {:.4}",
        mae
    );
