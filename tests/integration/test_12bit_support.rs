...
fn test_12bit_lossless_ct_pattern() {
    ...
    let decoded_u8 = image.reconstruct_pixels().expect("12-bit CT reconstruction failed");
    let decoded_u16 = u8_to_u16_le(&decoded_u8);
...
fn test_12bit_lossless_checkerboard() {
    ...
    let mut reader = JpegStreamReader::new(&j2k_buffer[..j2k_size]);
    let mut decoder = J2kDecoder::new(&mut reader);
    let image = decoder.decode().expect("12-bit checkerboard decoding failed");
    let decoded_u8 = image.reconstruct_pixels().expect("12-bit checkerboard reconstruction failed");
    let decoded_u16 = u8_to_u16_le(&decoded_u8);
...
fn test_12bit_lossy_q100() {
    ...
    let decoded_u8 = image.reconstruct_pixels().expect("12-bit lossy reconstruction failed");
    let decoded_u16 = u8_to_u16_le(&decoded_u8);
    ...
    assert!(mae < 50.0, "12-bit Q100 should have MAE < 50, got {:.4}", mae);
    
    // Calculate PSNR
    if mae > 0.0 {
        let mse = pixels_u16.iter().zip(decoded_u16.iter())
            .map(|(a, b)| {
        ...
        assert!(psnr > 20.0, "12-bit Q100 PSNR should be > 20 dB, got {:.2}", psnr);
    }
...
fn test_12bit_lossy_q85() {
    ...
    let decoded_u8 = image.reconstruct_pixels().expect("12-bit Q85 reconstruction failed");
    let decoded_u16 = u8_to_u16_le(&decoded_u8);
    ...
    assert!(mae < 100.0, "12-bit Q85 should have MAE < 100, got {:.4}", mae);
