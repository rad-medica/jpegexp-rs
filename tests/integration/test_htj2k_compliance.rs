...
fn test_htj2k_dicom_compliance_markers() {
    ...
    loop {
        ...
        if reader.remaining_data().is_empty() {
            break;
        }
    }
    ...
    assert!(
        found_cap,
        "CAP marker (0xFF50) missing - Required for HTJ2K"
    );
    ...
    assert_eq!(
        pcap_val & ht_bit,
        ht_bit,
        "CAP marker Pcap must have HTJ2K bit set (0x{:08X})",
        pcap_val
    );

    println!(
        "DICOM HTJ2K Compliance Check Passed: CAP present, Pcap=0x{:08X}",
        pcap_val
    );
}
    ...
    loop {
        if reader.read_u8().unwrap() == 0xFF && reader.read_u8().unwrap() == 0x52 {
            ...
            assert_eq!(
                trans,
                1,
                "Transformation must be 1 (5-3 Reversible) for Lossless HTJ2K"
            );
            break;
        }
        if reader.remaining_data().is_empty() {
