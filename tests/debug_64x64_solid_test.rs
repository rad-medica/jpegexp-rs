...
fn debug_64x64_solid_5_levels() {
    ...
    println!("\n=== Testing 64x64 SOLID (128) with 5 decomposition levels ===\n");
    ...
    fs::write("debug_ours_64x64_solid.j2k", our_bytes).unwrap();
    
    fs::write("debug_input_64x64_solid.raw", &pixels).unwrap();
    let status = Command::new("libs/bin/opj_compress.exe")
        .args(&[
            "-i", "debug_input_64x64_solid.raw",
            "-o", "debug_opj_64x64_solid.j2k",
            "-n", "6",
            "-r", "1",
            "-F", "64,64,1,8,u",
            "-I",
    ...
    let opj_bytes = fs::read("debug_opj_64x64_solid.j2k").unwrap();
    
    println!("--- File Sizes ---");
    println!("Ours:     {} bytes", our_size);
    println!("OpenJPEG: {} bytes", opj_bytes.len());
    
    let our_decoded_pnm = "debug_ours_decoded_64x64_solid.pnm";
    let opj_decoded_pnm = "debug_opj_decoded_64x64_solid.pnm";
    
    let status1 = Command::new("libs/bin/opj_decompress.exe")
        .args(&["-i", "debug_ours_64x64_solid.j2k", "-o", our_decoded_pnm])
        .output()
        .expect("Failed to decode our file");
    
    let status2 = Command::new("libs/bin/opj_decompress.exe")
        .args(&["-i", "debug_opj_64x64_solid.j2k", "-o", opj_decoded_pnm])
        .output()
    ...
    assert_eq!(mae_ours, 0.0, "Solid pattern should be perfectly lossless!");
}
