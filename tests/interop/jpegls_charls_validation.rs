...
mod jpegls_decoder_validation {
    ...
    fn test_charls_decode(name: &str, width: u32, height: u32, components: u8, bit_depth: u8) {
        ...
        let jls_path = format!("tests/fixtures/jpegls/{}.jls", name);
        let jls_data =
            fs::read(&jls_path).unwrap_or_else(|e| panic!("Failed to read {}: {}", jls_path, e));

        // Load the expected raw data
        let raw_path = format!("tests/fixtures/jpegls/{}.raw", name);
        let expected_data =
        ...
        match decoder.decode(&mut decoded_data) {
            Ok(_) => {
                ...
                if decoded_data == expected_data {
                    ...
                    let total_diff: i64 = decoded_data
                        .iter()
                        .zip(expected_data.iter())
                    ...
                    // Find first mismatch
                    let first_mismatch = decoded_data
                        .iter()
                        .zip(expected_data.iter())
                        .enumerate()
                        .find(|(_, (a, b))| a != b);
    ...
    fn test_images_exist() {
        ...
        let test_dir = Path::new("tests/fixtures/jpegls");
        assert!(
            test_dir.exists(),
            "Test images directory not found. Run: python3 tests/scripts/generate_jpegls_test_images.py"
        );

        let readme = test_dir.join("README.md");
        assert!(
            readme.exists(),
            "README.md not found in test images directory"
        );
        ...
        assert!(
            tiny_gray.exists(),
            "Test images not generated. Run: python3 tests/scripts/generate_jpegls_test_images.py"
        );
    }
}
