...
fn write_pgm_ppm(
    ...
    if depth > 8 {
        for &v in pixels {
            f.write_all(&[(v >> 8) as u8, (v & 0xFF) as u8])?;
        }
    } else {
        for &v in pixels {
            f.write_all(&[v as u8])?;
...
fn read_pgm_ppm(path: &str) -> std::io::Result<(Vec<u16>, u32, u32, u32)> {
    ...
    if maxval > 255 {
        for chunk in pixel_data.chunks(2) {
            if chunk.len() == 2 {
                pixels.push(((chunk[0] as u16) << 8) | (chunk[1] as u16));
            }
        }
    } else {
        for &b in pixel_data {
...
fn test_final_interop() {
    ...
    let j2k_path = "tests/fixtures/final_interop_A.j2k";
    std::fs::write(j2k_path, &encoded[..len]).unwrap();
    println!("  Encoded {} bytes to {}", len, j2k_path);

    let out_pgm = "tests/fixtures/final_interop_A.pgm";
    ...
    if !output.status.success() {
        eprintln!(
            "OpenJPEG decompress stderr: {}",
            String::from_utf8_lossy(&output.stderr)
        );
        panic!(
            "OpenJPEG decompress failed with exit code: {:?}",
            output.status.code()
        );
    }
    ...
    assert_eq!(
        mae_a,
        0.0,
        "jpegexp encoder should produce files decodable by OpenJPEG with MAE=0"
    );

    // 2. OpenJPEG -> jpegexp (Lossless)
    println!("Test 2: OpenJPEG encoder -> jpegexp decoder");
    let src_pgm = "tests/fixtures/final_interop_B_src.pgm";
    write_pgm_ppm(src_pgm, &original_u16, w, h, c, depth as u8).unwrap();
    let j2k_path_b = "tests/fixtures/final_interop_B.j2k";
    ...
    if !output.status.success() {
        eprintln!(
            "OpenJPEG compress stderr: {}",
            String::from_utf8_lossy(&output.stderr)
        );
        panic!(
            "OpenJPEG compress failed with exit code: {:?}",
            output.status.code()
        );
    }
    ...
    assert_eq!(
        mae_b,
        0.0,
        "jpegexp decoder should decode OpenJPEG files with MAE=0"
    );
