...
fn test_openjpeg_can_decode_our_lossless() {
    ...
    // Write J2K file
    std::fs::write("tests/openjpeg_compat/test_lossless.j2k", &encoded)
        .expect("Failed to write J2K");
    
    // Decode with OpenJPEG
    let output = Command::new(OPJ_DECOMPRESS)
        .args([
            "-i", "tests/openjpeg_compat/test_lossless.j2k",
            "-o", "tests/openjpeg_compat/test_lossless_opj.pgm",
        ])
        .output()
...
fn test_we_can_decode_openjpeg_lossless() {
    ...
    create_pgm("tests/openjpeg_compat/test_src.pgm", width, height, &original)
    ...
    let output = Command::new(OPJ_COMPRESS)
        .args([
            "-i", "tests/openjpeg_compat/test_src.pgm",
            "-o", "tests/openjpeg_compat/test_opj_lossless.j2k",
    ...
    // Decode with jpegexp-rs
    let j2k_data = std::fs::read("tests/openjpeg_compat/test_opj_lossless.j2k")
    ...
    // Cleanup
    let _ = std::fs::remove_file("tests/openjpeg_compat/test_src.pgm");
    let _ = std::fs::remove_file("tests/openjpeg_compat/test_opj_lossless.j2k");
}
