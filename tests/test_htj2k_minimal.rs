...
fn calculate_mae(img1: &[u8], img2: &[u8]) -> f64 {
    ...
    let sum: i64 = img1.iter().zip(img2.iter())
...
fn test_htj2k_minimal_2x2_black() {
    ...
    save_pgm("test_2x2_black.pgm", &pixels, width, height).unwrap();
    
    let output = Command::new("./open_htj2k_enc.exe")
        .args(&["-i", "test_2x2_black.pgm", "-o", "test_2x2_black.j2c", "Creversible=yes"])
        .env("HTJ2K_DEBUG", "1")
        .output()
        .expect("Failed to run encoder");
    
    if !output.status.success() {
        eprintln!("Encoder failed: {}", String::from_utf8_lossy(&output.stderr));
        panic!("Encoder failed");
    }
    
    println!("Encoded file size: {} bytes", fs::metadata("test_2x2_black.j2c").unwrap().len());
    
    // Decode with our decoder
    let encoded_data = fs::read("test_2x2_black.j2c").unwrap();
    println!("Encoded data (hex): {}", hex::encode(&encoded_data[..encoded_data.len().min(100)]));
    ...
    fs::remove_file("test_2x2_black.pgm").ok();
    fs::remove_file("test_2x2_black.j2c").ok();
...
fn test_htj2k_minimal_2x2_white() {
    ...
    save_pgm("test_2x2_white.pgm", &pixels, width, height).unwrap();
    
    let output = Command::new("./open_htj2k_enc.exe")
        .args(&["-i", "test_2x2_white.pgm", "-o", "test_2x2_white.j2c", "Creversible=yes"])
        .env("HTJ2K_DEBUG", "1")
        .output()
        .expect("Failed to run encoder");
    ...
    let encoded_data = fs::read("test_2x2_white.j2c").unwrap();
    let mut reader = JpegStreamReader::new(&encoded_data);
    let mut decoder = J2kDecoder::new(&mut reader);
    ...
    fs::remove_file("test_2x2_white.pgm").ok();
    fs::remove_file("test_2x2_white.j2c").ok();
...
fn test_htj2k_minimal_2x2_checker() {
    ...
    save_pgm("test_2x2_checker.pgm", &pixels, width, height).unwrap();
    
    let output = Command::new("./open_htj2k_enc.exe")
        .args(&["-i", "test_2x2_checker.pgm", "-o", "test_2x2_checker.j2c", "Creversible=yes"])
        .env("HTJ2K_DEBUG", "1")
        .output()
        .expect("Failed to run encoder");
    ...
    let encoded_data = fs::read("test_2x2_checker.j2c").unwrap();
    ...
    fs::remove_file("test_2x2_checker.pgm").ok();
    fs::remove_file("test_2x2_checker.j2c").ok();
...
fn test_htj2k_minimal_4x4_gradient() {
    ...
    save_pgm("test_4x4_grad.pgm", &pixels, width, height).unwrap();
    
    let output = Command::new("./open_htj2k_enc.exe")
        .args(&["-i", "test_4x4_grad.pgm", "-o", "test_4x4_grad.j2c", "Creversible=yes", "Clevels=0"])
        .env("HTJ2K_DEBUG", "1")
        .output()
        .expect("Failed to run encoder");
    ...
    let encoded_data = fs::read("test_4x4_grad.j2c").unwrap();
    println!("Encoded size: {} bytes", encoded_data.len());
    ...
    fs::remove_file("test_4x4_grad.pgm").ok();
    fs::remove_file("test_4x4_grad.j2c").ok();
...
fn test_our_htj2k_encoder_with_openhtj2k_decoder() {
    ...
    fs::write("test_our_htj2k.j2c", &output).unwrap();
    
    // Decode with OpenHTJ2K
    let decode_output = Command::new("./open_htj2k_dec.exe")
        .args(&["-i", "test_our_htj2k.j2c", "-o", "test_our_htj2k_decoded.pgm"])
        .output()
        .expect("Failed to run decoder");
    
    if !decode_output.status.success() {
        eprintln!("OpenHTJ2K decoder stderr: {}", String::from_utf8_lossy(&decode_output.stderr));
        eprintln!("OpenHTJ2K decoder stdout: {}", String::from_utf8_lossy(&decode_output.stdout));
        
        fs::remove_file("test_our_htj2k.j2c").ok();
        panic!("OpenHTJ2K decoder failed on our HTJ2K encoded file");
    }
    
    // OpenHTJ2K adds _00 suffix to output filename for first component
    let decoded_path = if Path::new("test_our_htj2k_decoded_00.pgm").exists() {
        "test_our_htj2k_decoded_00.pgm"
    } else {
        "test_our_htj2k_decoded.pgm"
    };
    ...
    fs::remove_file("test_our_htj2k.j2c").ok();
    fs::remove_file("test_our_htj2k_decoded.pgm").ok();
    fs::remove_file("test_our_htj2k_decoded_00.pgm").ok();
    
    assert_eq!(mae, 0.0, "Our HTJ2K encoder should be compatible with OpenHTJ2K decoder");
}
...
fn test_decode_openhtj2k_2x2_black() {
    ...
    println!("First 60 bytes (hex): {}", hex::encode(&data[..60.min(data.len())]));
    ...
    if let Some(cap) = &image.cap {
        println!("Pcap: 0x{:08X}, is_htj2k: {}", cap.pcap, (cap.pcap & 0x00020000) != 0);
    }
