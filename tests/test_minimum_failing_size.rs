...
fn test_find_minimum_failing_size() {
    ...
    for size in sizes {
        ...
        let encoded_len = match encoder.encode(&pixels, &frame_info, &mut encoded) {
            Ok(len) => len,
            Err(_) => {
                println!("{}x{}: SKIPPED (too small for DWT level {})", size, size, dwt_level);
                continue;
            }
        };
        ...
        if error > 0.01 && size > 2 {
            ...
            println!("  First pixel: expected R={}, G={}, B={}", 
                     pixels[0], pixels[1], pixels[2]);
            println!("  First pixel: got      R={}, G={}, B={}", 
                     decoded[0], decoded[1], decoded[2]);
            println!("  Codeblock size: 64x64 (fixed)");
            println!("  After {} DWT levels, LL subband is {}x{}", 
                     dwt_level, size >> dwt_level, size >> dwt_level);
        }
    }
}
