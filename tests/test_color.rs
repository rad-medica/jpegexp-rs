...
fn test_color_rgb_lossless() {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            // Mix
            r = if x >= width/2 && y >= height/2 { 255 } else { r };
            g = if x >= width/2 && y >= height/2 { 255 } else { g };
            b = if x >= width/2 && y >= height/2 { 255 } else { b };
    ...
    println!("Compression ratio: {:.2}x", (original.len() as f64) / (encoded_len as f64));
...
fn test_color_rgb_12bit_lossless() {
    ...
    for i in 0..original_u16.len() {
        ...
        if val_orig != val_recon {
            diffs += 1;
            if diffs < 10 {
                println!("Mismatch at sample {}: orig={}, recon={}", i, val_orig, val_recon);
            }
        }
    }
    
    assert_eq!(diffs, 0, "12-bit RGB roundtrip failed with {} mismatches", diffs);
}
