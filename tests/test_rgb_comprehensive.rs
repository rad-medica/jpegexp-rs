    ...
    for size in sizes {
        ...
        if mae_avg > 0.01 {
            ...
            for i in 0..(size * size) as usize {
                for c in 0..3 {
                    let idx = i * 3 + c;
                    if original[idx] != reconstructed[idx] && shown < 10 {
                        let comp = ["R", "G", "B"][c];
                        println!("    Pixel {} {}: orig={}, recon={}", 
                                 i, comp, original[idx], reconstructed[idx]);
                        shown += 1;
                    }
                }
            }
...
fn test_rgb_dwt_levels() {
    ...
    for dwt_level in 0..=5 {
        ...
        for i in 0..original.len() {
            let error = (original[i] as i32 - reconstructed[i] as i32).abs();
            total_error += error as f64;
        }
        let mae = total_error / original.len() as f64;
