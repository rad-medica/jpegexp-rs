...
const MQ_TABLE: [MqState; 47] = [
    ...
    MqState { qe: 0x2401, nmps: 23, nlps: 20, switch: 0 },
    MqState { qe: 0x2201, nmps: 24, nlps: 21, switch: 0 },
    MqState { qe: 0x1c01, nmps: 25, nlps: 22, switch: 0 },
    MqState { qe: 0x1801, nmps: 26, nlps: 23, switch: 0 },
    MqState { qe: 0x1601, nmps: 27, nlps: 24, switch: 0 },
    MqState { qe: 0x1401, nmps: 28, nlps: 25, switch: 0 },
    MqState { qe: 0x1201, nmps: 29, nlps: 26, switch: 0 },
    MqState { qe: 0x1101, nmps: 30, nlps: 27, switch: 0 },
    MqState { qe: 0x0ac1, nmps: 31, nlps: 28, switch: 0 },
    MqState { qe: 0x09c1, nmps: 32, nlps: 29, switch: 0 },
    MqState { qe: 0x08a1, nmps: 33, nlps: 30, switch: 0 },
    MqState { qe: 0x0521, nmps: 34, nlps: 31, switch: 0 },
    MqState { qe: 0x0441, nmps: 35, nlps: 32, switch: 0 },
    MqState { qe: 0x02a1, nmps: 36, nlps: 33, switch: 0 },
    MqState { qe: 0x0221, nmps: 37, nlps: 34, switch: 0 },
    MqState { qe: 0x0141, nmps: 38, nlps: 35, switch: 0 },
    MqState { qe: 0x0111, nmps: 39, nlps: 36, switch: 0 },
    MqState { qe: 0x0085, nmps: 40, nlps: 37, switch: 0 },
    MqState { qe: 0x0049, nmps: 41, nlps: 38, switch: 0 },
    MqState { qe: 0x0025, nmps: 42, nlps: 39, switch: 0 },
    MqState { qe: 0x0015, nmps: 43, nlps: 40, switch: 0 },
    MqState { qe: 0x0009, nmps: 44, nlps: 41, switch: 0 },
    MqState { qe: 0x0005, nmps: 45, nlps: 42, switch: 0 },
    MqState { qe: 0x0001, nmps: 45, nlps: 43, switch: 0 },
...
impl MqCoder {
    ...
    pub fn set_context(&mut self, cx: usize, state: u8) {
        if cx >= self.contexts.len() { self.contexts.resize(cx + 1, 0); }
    ...
    pub fn encode(&mut self, d: u8, cx: usize) {
        ...
        if d == mps {
            if (self.a & 0x8000) == 0 {
                if self.a < qe { self.a = qe; } else { self.c += qe; }
            ...
            if self.a < qe { self.c += qe; } else { self.a = qe; }
    ...
    pub fn flush(&mut self) {
        ...
        self.c |= 0xffff;
        if self.c >= temp_c { self.c -= 0x8000; }
        self.c <<= self.ct;
        self.byte_out();
        self.c <<= self.ct;
        self.byte_out();
    ...
    fn byte_in(&mut self) {
        if self.src_pos < self.source.len() {
            if self.source[self.src_pos] == 0xff {
                let next = if self.src_pos + 1 < self.source.len() { self.source[self.src_pos + 1] } else { 0xff };
                if next > 0x8f {
                    self.c += 0xff00;
                    self.ct = 8;
                } else {
