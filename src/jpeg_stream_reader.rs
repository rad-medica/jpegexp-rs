use crate::coding_parameters::{
    CodingParameters, JpeglsPcParameters,
};
use crate::error::JpeglsError;
use crate::jpeg_marker_code::{JpegMarkerCode, JPEG_MARKER_START_BYTE};
use crate::{
    FrameInfo, InterleaveMode, SpiffHeader,
};
...
impl<'a> JpegStreamReader<'a> {
    ...
    pub fn read_header(&mut self, spiff_header: &mut Option<SpiffHeader>) -> Result<(), JpeglsError> {
    ...
    fn read_spiff_header_segment(&mut self) -> Result<Option<SpiffHeader>, JpeglsError> {
        ...
        for i in 0..6 { identifier[i] = self.read_byte()?; }
