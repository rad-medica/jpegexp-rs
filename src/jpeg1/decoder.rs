...
impl<'a> Jpeg1Decoder<'a> {
    ...
    pub fn decode_u16(&mut self, destination: &mut [u16]) -> Result<(), JpeglsError> {
        ...
        for (c, coeff_buf) in coefficient_buffers.iter().enumerate().take(components_count) {
            ...
            for b in 0..(comp_blocks_w * comp_blocks_h) {
                ...
                if block_offset + 64 <= coeff_buf.len() {
                    ...
                    crate::jpeg1::dct::idct_8x8_baseline(&dequant_coeffs, &mut idct_out);
        ...
        let max_val = ((1 << frame_info.bits_per_sample) - 1) as f32;
            ...
            for px in 0..width {
                if components_count == 1 {
                    ...
                    if block_idx < component_buffers_f32[0].len() {
                        let val = (component_buffers_f32[0][block_idx] + level_shift)
                            .round()
                    ...
                    let r = y_val + 1.402 * cr_val + level_shift;
                    let g = y_val - 0.344136 * cb_val - 0.714136 * cr_val + level_shift;
                    let b = y_val + 1.772 * cb_val + level_shift;

                    let pixel_idx = (py * width + px) * 3;
                    if pixel_idx + 2 < destination.len() {
                        destination[pixel_idx] = r.clamp(0.0, max_val) as u16;
                        destination[pixel_idx + 1] = g.clamp(0.0, max_val) as u16;
                        destination[pixel_idx + 2] = b.clamp(0.0, max_val) as u16;
                    }
                }
            }
        }
        ...
        for py in 0..height {
            for px in 0..width {
                if components_count == 1 {
                    ...
                    let pixel_idx = (py * width + px) * 3;
                    if pixel_idx + 2 < destination.len() {
                        destination[pixel_idx] = r.clamp(0.0, max_val) as u8;
                        destination[pixel_idx + 1] = g.clamp(0.0, max_val) as u8;
                        destination[pixel_idx + 2] = b.clamp(0.0, max_val) as u8;
                    }
                }
            }
        }
    ...
    fn decode_lossless_u16(&mut self, destination: &mut [u16]) -> Result<(), JpeglsError> {
        let frame_info = self.reader.frame_info();
        let width = frame_info.width as usize;
        let height = frame_info.height as usize;
        let components_count = self.reader.components.len();
        ...
        for py in 0..height {
            for px in 0..width {
                ...
                for c in 0..components_count {
                    ...
                    let val =
                        component_pixels[c][py * width + px].clamp(0, (1 << bit_depth) - 1) as u16;
                    if pixel_idx + c < destination.len() {
                        destination[pixel_idx + c] = val;
                    }
                }
