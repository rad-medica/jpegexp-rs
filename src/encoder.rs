...
impl<'a> JpeglsEncoder<'a> {
    ...
    pub fn set_preset_coding_parameters(&mut self, pc_parameters: JpeglsPcParameters) -> Result<(), JpeglsError> {
         self.pc_parameters = Some(pc_parameters);
         Ok(())
    }

    pub fn encode(&mut self, source: &[u8]) -> Result<usize, JpeglsError> {
         let frame_info = *self.frame_info.as_ref().ok_or(JpeglsError::InvalidParameterComponentCount)?;
         ...
         let coding_parameters = CodingParameters {
              ...
              restart_interval: 0,
              limit: compute_limit_parameter(frame_info.bits_per_sample, self.near_lossless, frame_info.component_count), 
         ...
         let _ = self.writer.write_start_of_scan_segment(component_count, self.near_lossless, self.interleave_mode)?;
    ...
    fn encode_scan_typed<T: crate::traits::JpeglsSample>(&mut self, source: &[T], frame_info: &FrameInfo, pc: JpeglsPcParameters, coding_params: CodingParameters) -> Result<usize, JpeglsError> {
